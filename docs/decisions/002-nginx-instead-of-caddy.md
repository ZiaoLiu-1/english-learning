...
- 日期：2026-07-07
- 状态：gary 已批（2026-07-07），附三个执行条件：① location 块加 `client_max_body_size 25m`（M2 录音上传，默认 1M 必 413）；② `/english/audio/` 用 nginx alias 直接 serve 静态音频，不过 Next；③ whisper 请求走异步任务，不靠拉长 proxy_read_timeout
